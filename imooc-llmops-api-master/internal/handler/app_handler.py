...
class AppHandler:
    ...
    async def create_app(self, request: Request, current_user: Account = Depends(get_current_user)):
        """调用服务创建新的APP记录"""
        # 1.从request中获取表单数据并校验
        form_data = await request.form()
        req = CreateAppReq(formdata=form_data)
        if not req.validate():
    ...
    async def get_app(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
        """获取指定的应用基础信息"""
        app = self.app_service.get_app(app_id, current_user)
        resp = GetAppResp()
        return success_json(resp.dump(app))

    async def update_app(self, app_id: UUID, request: Request, current_user: Account = Depends(get_current_user)):
        """根据传递的信息更新指定的应用"""
        # 1.从request中获取表单数据并校验
        form_data = await request.form()
        req = UpdateAppReq(formdata=form_data)
        if not req.validate():
            return validate_error_json(req.errors)
    ...
    async def copy_app(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id快速拷贝该应用"""
        app = self.app_service.copy_app(app_id, current_user)
        return success_json({"id": app.id})

    async def delete_app(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
        """根据传递的信息删除指定的应用"""
        self.app_service.delete_app(app_id, current_user)
        return success_message("删除Agent智能体应用成功")

    async def get_apps_with_page(self, request: Request, current_user: Account = Depends(get_current_user)):
        """获取当前登录账号的应用分页列表数据"""
        # 1.提取数据并校验
        req = GetAppsWithPageReq(dict(request.query_params))
        if not req.validate():
            return validate_error_json(req.errors)
    ...
    async def get_draft_app_config(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
    ...
    async def update_draft_app_config(self, app_id: UUID, request: Request, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id+草稿配置更新应用的最新草稿配置"""
        # 1.获取草稿请求json数据
        draft_app_config = await request.json()
    ...
    async def publish(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id发布/更新特定的草稿配置信息"""
        self.app_service.publish_draft_app_config(app_id, current_user)
        return success_message("发布/更新应用配置成功")

    async def cancel_publish(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id，取消发布指定的应用配置信息"""
        self.app_service.cancel_publish_app_config(app_id, current_user)
        return success_message("取消发布应用配置成功")

    async def fallback_history_to_draft(self, app_id: UUID, request: Request, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id+历史配置版本id，退回指定版本到草稿中"""
        # 1.从request中获取表单数据并校验
        form_data = await request.form()
        req = FallbackHistoryToDraftReq(formdata=form_data)
        if not req.validate():
            return validate_error_json(req.errors)
    ...
    async def get_publish_histories_with_page(self, app_id: UUID, request: Request, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id，获取应用发布历史列表"""
        # 1.获取请求数据并校验
        req = GetPublishHistoriesWithPageReq(dict(request.query_params))
        if not req.validate():
            return validate_error_json(req.errors)
    ...
    async def get_debug_conversation_summary(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id获取调试会话长期记忆"""
        summary = self.app_service.get_debug_conversation_summary(app_id, current_user)
        return success_json({"summary": summary})

    async def update_debug_conversation_summary(self, app_id: UUID, request: Request, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id+摘要信息更新调试会话长期记忆"""
        # 1.从request中获取表单数据并校验
        form_data = await request.form()
        req = UpdateDebugConversationSummaryReq(formdata=form_data)
        if not req.validate():
            return validate_error_json(req.errors)
    ...
    async def delete_debug_conversation(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id，清空该应用的调试会话记录"""
        self.app_service.delete_debug_conversation(app_id, current_user)
        return success_message("清空应用调试会话记录成功")

    async def debug_chat(self, app_id: UUID, request: Request, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id+query，发起调试对话"""
        # 1.从request中获取表单数据并校验
        form_data = await request.form()
        req = DebugChatReq(formdata=form_data)
        if not req.validate():
            return validate_error_json(req.errors)
    ...
    async def stop_debug_chat(self, app_id: UUID, task_id: UUID, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id+任务id停止某个应用的指定调试会话"""
        self.app_service.stop_debug_chat(app_id, task_id, current_user)
        return success_message("停止应用调试会话成功")

    async def get_debug_conversation_messages_with_page(self, app_id: UUID, request: Request, current_user: Account = Depends(get_current_user)):
        """根据传递的应用id，获取该应用的调试会话分页列表记录"""
        # 1.提取请求并校验数据
        req = GetDebugConversationMessagesWithPageReq(dict(request.query_params))
        if not req.validate():
            return validate_error_json(req.errors)
    ...
    async def get_published_config(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
    ...
    async def regenerate_web_app_token(self, app_id: UUID, current_user: Account = Depends(get_current_user)):
