...
class AssistantAgentHandler:
    ...
    async def assistant_agent_chat(self):
        """与辅助智能体进行对话聊天"""
        # 1.提取请求数据并校验
    ...
    async def stop_assistant_agent_chat(self, task_id: UUID):
        """停止与辅助智能体的对话聊天"""
        self.assistant_agent_service.stop_chat(task_id, current_user)
        return success_message("停止辅助Agent会话成功")
    async def get_assistant_agent_messages_with_page(self):
        """获取与辅助智能体的消息分页列表"""
        # 1.提取请求并校验数据
    ...
    async def delete_assistant_agent_conversation(self):
        """清空/删除与辅助智能体的聊天会话记录"""
        # 1.调用服务清空辅助Agent会话列表
