...
class WorkflowHandler:
    ...
    async def update_workflow(self, workflow_id: UUID):
        """根据传递的工作流id获取工作流详情"""
        # 1.提取请求并校验
    ...
    async def get_workflows_with_page(self):
        """获取当前登录账号下的工作流分页列表数据"""
        # 1.提取请求并校验
    ...
    async def update_draft_graph(self, workflow_id: UUID):
    ...
    async def get_draft_graph(self, workflow_id: UUID):
    ...
    async def debug_workflow(self, workflow_id: UUID):
    ...
    async def cancel_publish_workflow(self, workflow_id: UUID):
